...
schema_final = StructType([
    StructField("sk_dim_item_receita", StringType(), True),
...
if arg == 'setup':
    if not DeltaTable.isDeltaTable(spark, path_refined):
        ...
        df_vazio.write \
            .format("delta") \
            .save(path_refined)
        print("Tabela Delta 'dim_item_receita' inicializada do zero com Schema estruturado!")
    else:
...
# --- MODO RUN ---
    ...
    try:
        ...
        # Populando a SK com hash MD5 baseada no id_item e adicionando data de alteração
        df_updates = df_csv \
            .withColumn("sk_dim_item_receita", md5(concat(col("id_item").cast(StringType()), lit("")))) \
            .withColumn("dt_alteracao", current_timestamp()) \
            .select("sk_dim_item_receita", "id_item", "cd_item", "nome", "dt_alteracao")

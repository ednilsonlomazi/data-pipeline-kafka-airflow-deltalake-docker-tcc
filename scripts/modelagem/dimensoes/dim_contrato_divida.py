...
schema_final = StructType([
    StructField("sk_dim_contrato_divida", StringType(), True),
    StructField("id_contrato", IntegerType(), True),
    StructField("cd_siafi", StringType(), True),  
...
if arg == 'setup':
    if not DeltaTable.isDeltaTable(spark, path_refined):
        ...
        df_vazio.write \
            .format("delta") \
            .save(path_refined)
        print("Tabela Delta 'dim_contrato_divida' inicializada do zero com Schema estruturado!")
    else:
...
# --- MODO RUN ---
    ...
    try:
        ...
        # Populando a SK com hash MD5 baseada no id_contrato e adicionando data de alteração
        df_updates = df_csv \
            .withColumn("sk_dim_contrato_divida", md5(concat(col("id_contrato").cast(StringType()), lit("")))) \
            .withColumn("dt_alteracao", current_timestamp()) \
            .select("sk_dim_contrato_divida", "id_contrato", "cd_siafi", "nr_contrato", "dt_alteracao")

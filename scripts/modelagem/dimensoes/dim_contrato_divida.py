...
path_csv = "/opt/airflow/data/financas/dm_contrato_divida.csv"
path_refined = "s3a://refined/dim_contrato_divida"
...
schema_csv = StructType([
    StructField("id_contrato", IntegerType(), True),
    StructField("cd_siafi", StringType(), True),  # String previne perda de formatação se houver zeros à esquerda
    StructField("nr_contrato", StringType(), True)
])
...
if arg == 'setup':
    if not DeltaTable.isDeltaTable(spark, path_refined):
        # Criamos um DataFrame vazio com o schema + metadados de auditoria
        ...
        df_vazio.write \
            .format("delta") \
            .save(path_refined)
        print("Tabela Delta 'dim_contrato_divida' inicializada com sucesso na camada Refined.")
    else:
    ...
    try:
        ...
        # Adiciona uma coluna de timestamp para sabermos quando a dimensão foi atualizada/inserida
        ...
        dt_refined.alias("dim").merge(
            ...
            "dim.id_contrato = upd.id_contrato"
        ) \
        .whenMatchedUpdateAll() \
        .whenNotMatchedInsertAll() \
        .execute()

        print("Processamento de atualização da dim_contrato_divida concluído com sucesso!")

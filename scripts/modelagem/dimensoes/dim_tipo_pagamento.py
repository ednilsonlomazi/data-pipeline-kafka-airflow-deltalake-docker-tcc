...
path_dim = "s3a://refined/dim_tipo_pagamento"
...
try:
    # gera um dataframe com desc_tipo_pagamento distintos
    df_source = spark.read.format("delta").load("s3a://trusted/tab_trafego") \
        .select("desc_tipo_pagamento").distinct()

    if not DeltaTable.isDeltaTable(spark, path_dim):
        
        # apenas uma variavel que é parte de uma "Window Funcion em paralelo ao SQL"
        windowSpec = Window.orderBy("desc_tipo_pagamento")

        # adiciona a coluna sk_dim_tipo_pagamento, que eh o numero da linha correspondente da tabela ordenada por desc_tipo_pagamento
        # adiciona timestamp para saber o horario do insert
        df_initial = df_source.withColumn("sk_dim_tipo_pagamento", F.row_number().over(windowSpec)) \
        ...
        df_new_records = df_source.join(dt_target.toDF(), "desc_tipo_pagamento", "left_anti")
        
        # se existir dado novo, insere
        if df_new_records.count() > 0:

            # pega o maior sk_dim da tabela 
            max_id = dt_target.toDF().agg(F.max("sk_dim_tipo_pagamento")).collect()[0][0]

            # cria a sk_dim adicionando a max_id, entao se o sk_dim maior eh 10, entao 10 + 1 (row_number) = 11 e por ai vai...
            windowSpec = Window.orderBy("desc_tipo_pagamento")
            df_new_records = df_new_records.withColumn("sk_dim_tipo_pagamento", F.row_number().over(windowSpec) + max_id)
            ...
            df_final_source = df_source.join(
                df_new_records.select("desc_tipo_pagamento", "sk_dim_tipo_pagamento"), 
                "desc_tipo_pagamento", 
                "left"
            )
        else:
            df_final_source = df_source.withColumn("sk_dim_tipo_pagamento", F.lit(None).cast("int"))

        dt_target.alias("target").merge(
            df_final_source.alias("source"),
            "target.desc_tipo_pagamento = source.desc_tipo_pagamento"
